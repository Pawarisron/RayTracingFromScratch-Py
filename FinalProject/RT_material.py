...
class Glossy(Material):
    def __init__(self, cAlbedo, glossiness) -> None:
        super().__init__()
        self.color_albedo = rtu.Color(cAlbedo.r(), cAlbedo.g(), cAlbedo.b())
        self.glossiness = glossiness
        # Limit glossiness to 0.0 - 1.0 range
        if self.glossiness > 1.0:
            self.glossiness = 1.0
        elif self.glossiness < 0.0:
            self.glossiness = 0.0

    def scattering(self, rRayIn, hHinfo):
        # Compute the perfect reflection direction
        reflected_direction = reflect(rtu.Vec3.unit_vector(rRayIn.getDirection()), hHinfo.getNormal())

        # Add randomness based on glossiness
        fuzz = 1.0 - self.glossiness  # Inverse of glossiness for fuzz effect
        scattered_direction = reflected_direction + rtu.Vec3.random_vec3_unit() * fuzz

        # Generate scattered ray
        scattered_ray = rtr.Ray(hHinfo.getP(), scattered_direction, rRayIn.getTime())

        # Calculate BRDF for color attenuation
        attenuation_color = self.BRDF(rRayIn, scattered_ray, hHinfo)

        # Ensure the scattered direction is not below the surface
        if rtu.Vec3.dot_product(scattered_direction, hHinfo.getNormal()) < 1e-8:
            attenuation_color = rtu.Color(0, 0, 0)
